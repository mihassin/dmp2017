...
class CodeBook:
    ...
    # Index 12
    # min < val <= max
        self._mass = {'0 - 1000 kg': {'max': 1000},
                      '1000 - 1500 kg': {'min': 1000,
                                         'max': 1500},
                      '1500 - 2000 kg': {'min': 1500,
                                         'max': 2000},
                      'Greater than 2000 kg': {'min': 2000}}

    # Index 15
    # min < val <= max
        self._length = {'0 - 4300 mm': {'max': 4300},
                        '4300 - 4700 mm': {'min': 4300,
                                           'max': 4700},
                        'Longer than 4700 mm': {'min': 4700}}

    # Index 16
    # min < val <= max
        self._width = {'0 - 1700 mm': {'max': 1700},
                       '1700 - 1800 mm': {'min': 1700,
                                          'max': 1800},
                       '1800 - 1900 mm': {'min': 1800,
                                          'max': 1900},
                       '1900 - 2000 mm': {'min': 1900,
                                          'max': 2000},
                       'Wider than 2000 mm': {'min': 2000}}

    # Index 17
    # min < val <= max
        self._height = {'0 - 1450 mm': {'max': 1450},
                        '1450 - 1500 mm': {'min': 1450,
                                           'max': 1500},
                        '1500 - 1550 mm': {'min': 1500,
                                           'max': 1550},
                        'Greater than 1550 mm': {'min': 1550}}
    ...
    # Index 19
    # min < val <= max
        self._displacement = {'Small engine': {'max': 1000},
                              'Large engine': {'min': 2999},
                              '1000 - 1999 cc': {'min': 999,
                                                 'max': 2000},
                              '2000 - 2999 cc': {'min': 1999,
                                                 'max': 3000}}

    # Index 20
    # min < val <= max
        self._power = {'0 - 50 kW': {'max': 50},
                       '50 - 75 kW': {'min': 50,
                                      'max': 75},
                       '75 - 100 kW': {'min': 75,
                                       'max': 100},
    ...
    # Index 33
    # min < val <= max
        self._co2 = {'0 - 150 g': {'max': 150},
                     '150 - 200 g': {'min': 150,
                                     'max': 200},
                     'More than 200 g': {'min': 200}}

    # Index 34
    # min < val <= max
        self._km = {'More than 200000 km': {'min': 100000,
                                            'max': 200000},
                    '0 - 100000 km': {'max': 100000},
                    '100000 - 200000 km': {'min': 200000}}

        self._classes = flatten([self._vclasses.keys(),
                                 self._init_regis,
                                 self._usages.keys(),
                                 self._commencement,
                                 self._colors.keys(),
    ...
    def classify(self, data):
        ...
        for row in data:
            ...
            classified.append(
                self.__handle_date_interval(
                    row[1], self._init_regis))
            classified.append(self.__handle_dict(row[2], self._usages))
            classified.append(
                self.__handle_date_interval(
                    row[3], self._commencement))
            ...
            classified.append(
                self.__handle_dict_interval(
                    row[12], self._displacement))
            classified.append(
                self.__handle_dict_interval(
                    row[13], self._power))
            classified.append(
                self.__handle_dict_interval(
                    row[14], self._cylinders))
    ...
    def stringify(self, data):
        '''To be used after data mining procedures.
        Returns integer values into more readable item names.

        data -- data matrix with integer values
        '''
        str_data = []
        for i in range(len(data)):
            row = data[i]
            str_data.append([])
            for j in range(len(row)):
                str_data[i].append(self._classes[row[j]])
        return str_data
