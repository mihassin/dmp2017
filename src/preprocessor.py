...
class Preprocessor:
    ...
    def store_frequent(self, frequent):
        '''Stores the frequent itemsets

        frequent -- frequent itemsets and their supports
        '''
        print('Storing frequent itemsets..')
        result_file = self._path + 'results_frequent_itemsets.txt'
        str_freq = self.stringify(frequent, 'frequent')
        str_freq = sorted(str_freq, key=lambda item: item[1], reverse=1)
        if os.path.isfile(result_file):
            os.remove(result_file)
        with open(result_file, 'w') as f:
            f.write('frequent itemset | support\n')
            for freq in str_freq:
                f.write(str(freq[0]) + str(' ') + str(freq[1]) + '\n')
        print('Results are in ' + result_file)


    def store_rules(self, rules, measure='confidence'):
        '''Stores the association rules

        rules -- association rules
        '''
        print('Storing association rules..')
        result_file = self._path + 'results_association_rules.txt'
        str_rules = self.stringify(rules, 'rules')
        str_rules = sorted(str_rules, key=lambda item: item[2], reverse=1)
        if os.path.isfile(result_file):
            os.remove(result_file)
        with open(result_file, 'w') as f:
            f.write('association rule | support | '+measure+'\n')
            for rule in str_rules:
                f.write(str(rule[0]) + str(' ') + str(rule[1]) + str(' ') + str(rule[2]) + '\n')
        print('Results are in ' + result_file)
